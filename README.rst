...
papyrus_ogcserver provides an easy and convenient method for embeding
a Mapnik OGCServer into Pyramid applications.

papyrus_ogcserver is based on papyrus_tilecache by Éric Lemoine
...
papyrus_ogcserver is installed using the applications ``setup.py``

    $ python papyrus_ogcserver/setup.py install 

Often you'll want to make papyrus_ogcserver a dependency of your Pyramid
application, which is done by adding ``papyrus_ogcserver`` to the
...
papyrus_ogcserver using the ``Configurator.include`` method::

    def main(global_config, **settings):

        config = Configurator(settings=settings)

        import papyrus_ogcserver
        config.include(papyrus_ogcserver)

That's it! The Pyramid application now exposes a Mapnik OGCServer service at
``/ogcserver``.

`Test URL <http://localhost:23758/ogcserver?LAYERS=world&FORMAT=image%2Fpng&SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&STYLES=&EXCEPTIONS=application%2Fvnd.ogc.se_inimage&SRS=EPSG%3A4326&BBOX=-180.0000000000001,-90,119.46385052802589,209.463850528026&WIDTH=256&HEIGHT=256`_
